...
daemon模式很早的时候用过，很麻烦，需要两头都专门建用户和密码，特别权限处理，专门开个服务，特别端口放开，后来就支持
rsync -e ssh 的方式走ssh了，再后来就直接默认over
ssh了，和scp相比，天生增量支持，而且处理符号连接更好，而scp好像还特别处理登录的限制。如果非要daemon模式，想到的只有一种可能，就是没有现存帐号或者要特别限制。
