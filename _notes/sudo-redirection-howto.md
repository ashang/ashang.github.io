...
It would ignore things after redirection ( `>` or `>>` ) or `&&`, `;;`, etc.

So `sudo ls -lR >> /to-file` can not work if the write permission is not given.

Workaround:

```shell
echo 'ls -lR >> /to-file ' | sudo sh
```
...
```shell
ls -lR  | sudo  tee /to-file
ls -lR  | sudo  tee -a /to-file
```

`tee` has option `-a` for `--append`.
...
>
>       * Allows easy transfer for admin rights, in a short term or long term period, by adding and removing users from the admin group, while  not  compromising the root account.
>
