---
date: 2014-08-06
update: 2020-05-02T22:08:46+08:00
title: "Hello systemd!"
tags: "system"
---
...
# systemd-analyze plot > /home/user/startup.svg
...
# vim /etc/fuse.conf #type a, uncomment the user_allow_other line and ESC :wq
# mkdir /mnt/yourmountingdir
# chown user:user /mnt/yourmountingdir
sshfs user@remotemachine.com:/home/user /mnt/yourmountingdir/ -o allow_other #Asks for host trusting and password
# fusermount -u /mnt/yourmountingdir/ #To disconnect and unmount
...
环境参数文件，该文件可以用EnvironmentFile字段读取。
...
Description: shim for systemd
 ...
 the systemd helpers without using the init service
Original-Maintainer: Steve Langasek <vorlon@debian.org>
W06.7 Sun Feb 08, 22:28:24 @2@-bash-4.3 aaron@seldlx9662.corpusers.net:/home/aaron/barn
$ dpkg -s systemd
Package: systemd
Status: install ok installed
Priority: optional
...
# https://www.zhihu.com/question/20576222/answer/55037889

最明显的影响就是万能的/etc/rc.conf不能用了，当fedora、ubuntu还在将/etc折腾得攻零八落的时候，ArchLinux用一个/etc/rc.conf将常用的locale、kernel、lvm、services等不同配置内容统一到一个配置文件中，完美的体现了Arch标榜的KISS原则，也和其它发行版有明显的差异化。
...
作者：QUICKFOX
链接：https://www.zhihu.com/question/20576222/answer/15578225
来源：知乎

换到systemd是桌面应用下的趋势。首先，前提是拿arch来配服务器的人不是很多，因此arch的目标用户有两种，一是求新的Geek（或准Geek），另一种是桌面应用者。第一，systemd 对于服务使用并行启动方式，在我的电脑上可以明显感觉到并行启动所带来的速度提升，这对于追求快速启动的桌面应用来说是很必要的。第二，systemd 对于 sysV 有更好的兼容性，initscripts 如果想使用 sysV 提供的高阶应用需要借助另外的设置；第三，systemd 可以自行解决服务间依赖性，但 initscripts 不便于做这点；initscripts 是从 BSD 借鉴来的方案，修改简便，这种简便性对于服务器来说十分适用，但桌面用户就不见得了。至少不应该让桌面用户考虑哪个服务应该启动在前，启动在后的问题；最后，不知道楼主尝试过 systemctl 没有？这个集中化管理的命令给我的印象还不错，systemctl enable 一个服务就 OK 了，完全不需要考虑这个服务要依赖啥。而且 systemctl 对桌面用户还提供了更友好的 UI 界面（AUR里就有），理应更方便些。


# https://www.zhihu.com/question/20576222/answer/17627821

维护成本降低，这个是十分显著的变化。在使用initscripts时，程序的启动脚本都是archlinux维护者自己写的，并且在其他发行版不能用，写起来也相对复杂。改用systemd后，.service文件是通用的，不用自己维护一份，同时写法简单，方便回馈上游。从功能上，systemd是更系统化的解决方案，考虑的方面要比initscripts多，复杂性也高，有性能优势。systemd让老archlinux用户不习惯的应该主要是分散的配置文件，实现细节隐藏得更深不便于深层次定制，二进制格式日志等，这些不能说是缺点，习惯了也就慢慢接受了。迁移systemd后，archlinux自己的东西更少了，netcfg发展成netctl可推广成一个通用工具，自己的东西基本只有pacman了，但其实这正是回归到简单，对简单的理解每个人是不同的，但现在archlinux从发行版角度看真可以算是最简单的了。
...
RequiredComponents=org.gnome.Shell;org.gnome.SettingsDaemon.A11ySettings;org.gnome.SettingsDaemon.Color;org.gnome.SettingsDaemon.Datetime;org.gnome.SettingsDaemon.Housekeeping;org.gnome.SettingsDaemon.Keyboard;o
rg.gnome.SettingsDaemon.MediaKeys;org.gnome.SettingsDaemon.Power;org.gnome.SettingsDaemon.PrintNotifications;org.gnome.SettingsDaemon.Rfkill;org.gnome.SettingsDaemon.ScreensaverProxy;org.gnome.SettingsDaemon.Sha
ring;org.gnome.SettingsDaemon.Smartcard;org.gnome.SettingsDaemon.Sound;org.gnome.SettingsDaemon.Wacom;org.gnome.SettingsDaemon.XSettings;

$ cat /usr/lib/systemd/user/gnome-session@ubuntu.target.d/ubuntu.session.conf
[Unit]
