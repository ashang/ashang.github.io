...
1.  `预处理`：源代码首先交由`预处理器`处理，其执行以`#`开头的指令
2.  `编译`：`编译器`会将经预处理过的代码翻译成机器指令，生成目标代码
3.  `链接`：由`链接器`将目标代码和所需的其他附加代码整合到一起，生成最终的可执行程序
...
### printf格式符
  ...
  - `%l`：按长整型量输出，`%ll`表示`long long int`或`unsigned long long
    int`
  - `%%`：输出百分号本身
...
### 输入输出常用函数
  ...
  - `fprintf`：向预定义流输出格式化字符串，第一个参数为流的名字，写成`stdout`就相当于`printf`。`fprintf(stdout,
    "%s, 你好", name);`
  - `sprintf`：向指定的字符串缓冲区输出格式化内容，第一个参数为缓冲区的名字。`sprintf(buffer, "$s,
    你好", name);`
...
`using`既可以引入整个命令空间，也可以引入单独的符号，如 `using std::string;`、`using
std::cin;`
...
2.  `extern const int num;`：可以被其他文件访问，引用时也使用`extern const int
num`
...
由于`char`实际上有`普通char`、`unsigned char`和`signed
char`三种类型，而编译器如何选择是未定义的，所以不要把`char`类型作为计算类型来使用，容易出问题

对于浮点数的选取，使用`double`类型基本上不会有错，应优先选用`double`，而不是`float`和`long
double`
...
输出操作符的返回值是输出流本身，所以`std::cout << 1+2 << std::endl;`可以写成`(std::cout
<< 1+2) << std::endl;`

输入操作符返回的也是输入流本身，如`std::cout << (std::cin >> a >>
b);`
...
`std::setfill()`：指定填充字符，在设置下一个不同的填充字符之前不会失效。`std::cout <<
std::setfill('*') << std::setw(2) << 2012 << 'n';`

`std::left`和`std::right`：控制输出左对齐或右对齐，在设置下一个不同的对齐方式之前不会失效。`std::cout
<< std::left << std::setw(2) << 2012 <<
'n';`
...
枚举值默认从`0`开始，也可以使用在定义时自定义赋值，允许存在相同的枚举值。如 `enum em_name {a = 2, b, c =
3, d};`，这样`a == 2`，`b == 3`，`c == 3`，`d ==
4`
...
1.  在`引用`变量使用了`const`修饰符的情况下，绑定对象可以为直接的数值、字符、字符串等，如 `const char
    &refVal = 'A';`
2.  `const引用`的绑定对象类型和`const引用`本身的类型可以不同，如 `int val = 65;`、`const
    char &refVal = val;`

-----

<red>date</red> :
2012-01-04、2012-01-29、2012-02-14、2012-07-26、2012-09-05、2012-09-09、2012-10-07合并整理
...
`main`函数中`return
0;`的作用有二：一是使`main`函数终止，从而结束程序的运行；二是指出`main`函数的返回值是`0`

`EOF`指代文件结尾，也就是`end of
    file`

`printf`格式符详解：

1.  `%d`或`%i`：以十进制形式输出带符号整数（正数不输出符号）
2.  `%c`：输出单个字符
3.  `%f`：以小数形式输出单、双精度浮点数
4.  `%e`：以指数形式输出单、双精度浮点数（使用小写`e`）
5.  `%E`：作用同`%e`，但输出的是大写`E`
6.  `%g`：从`%f`和`%e`中选宽度最短的输出单、双精度浮点数
7.  `%G`：作用同`%g`，但如果输出结果为指数形式，则使用大写`E`
8.  `%s`：输出字符串
9.  `%o`：以八进制形式输出无符号整数（不输出前缀`0`）
10. `%x`：以十六进制形式输出无符号整数（不输出前缀`0x`）
11. `%X`：作用同`%x`，但使用`A`、`B`、`C`、`D`、`E`、`F`等大写字母
12. `%u`：以十进制形式输出无符号整数
13. `%p`：输出地址数据（指针类型）
14. `%%`：输出百分号本身
15. `-`：输出结果左对齐，右边以空格填充（默认是右对齐，左边填空格）
16. `+`：对输出结果添加正负号
17. `#`：对于八进制和十六进制数据，输出前缀符号`0`和`0x`，同时对于`%g`、`%G`格式符，还能保证一定输出小数点
18. `h`：按短整型量输出
19. `l`：按长整型量输出
...
1.  `%d`：输入十进制整数
2.  `%c`：输入单个字符
3.  `%f`或`%e`：输入浮点数（包括小数形式和指数形式）
4.  `%s`：输入字符串
5.  `%o`：输入八进制整数
6.  `%x`：输入十六进制整数
7.  `%u`：输入无符号十进制整数
8.  `*`：将当前格式符所对应的输入数据及这个格式符本身忽略掉，转而处理下一个格式符和下一个输入数据
9.  `h`：按短整型量获取输入
10. `l`：按长整型量获取输入
11. `数字`：按指定宽度截取输入值，要注意的是只允许截取，不允许进行精度控制，也就是说这个数字不能是小数
